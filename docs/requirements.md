...
個人ナレッジについても、ユーザーが明示的に追加または許可したものだけを参照対象とする。
...
特に個人メモ、プロフィール、過去ログなどの個人情報は、ユーザーが明示的に許可しない限り外部送信してはならない。
...
MVP では、ユーザーはアプリ上で Markdown またはテキストファイルを明示的に追加する。

アプリは、追加されたファイルだけを個人ナレッジとして参照する。

ユーザーが明示的に追加していないローカルファイルやフォルダを自動で参照してはならない。

追加後は、アプリ内で以下を確認できる必要がある。
...
将来的に「リンクしたローカルフォルダを参照する機能」は追加してよいが、以下を満たす必要がある。

- ユーザーが明示的に選択したフォルダのみを対象にする
- デフォルト OFF
- 対象ファイル一覧を UI で確認できる
- 監視停止と解除ができる
...
検索対象は、アプリに明示的に追加された個人ナレッジに限定する。

ホームディレクトリ全体、Desktop 全体、Documents 全体などを無差別に検索対象にしてはならない。
...
- 個人ナレッジは明示追加型を標準とする
- 参照中のファイル一覧をユーザーが確認できる
- 個人ナレッジの削除時は、関連する索引やキャッシュも削除できる
- アプリが参照する範囲は最小権限に限定する
...
MVP では、個人ナレッジは「アプリ上で追加されたもの」を profiles 配下またはアプリ管理領域で扱うことを基本とする。

将来的に外部フォルダリンク機能を追加する場合も、参照対象は明示選択されたパスに限定する。
