...
whisper.cppと量子化済みWhisperモデルを使用する。Apple Siliconかつ総メモリ8GB以上、
またはその他のCPUで総メモリ16GB以上の場合は`large-v3-turbo-q5_0`、それ以外は
`base-q5_1`を推奨する。取得前にモデル名、容量、検出メモリ、選定理由を表示する。
モデルの取得、正確なサイズとハッシュの照合、状態確認、中止をアプリから行える。
モデル選定や取得のために音声を外部送信せず、生音声も保存しない。
